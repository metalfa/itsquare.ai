-- =============================================================================
-- Migration 011: Fix vector functions — no pg_extension manipulation
--
-- Migration 010 failed: UPDATE pg_extension requires superuser (not available
-- in Supabase). We don't need to move the extension back.
--
-- The vector extension is currently in the `extensions` schema (moved by 009).
-- The <=> operator is accessible as long as 'extensions' is in the search_path.
--
-- Fix: recreate all 4 vector functions with search_path = 'public, extensions'
-- Non-vector functions already correct from migration 009 (search_path = '').
-- =============================================================================
